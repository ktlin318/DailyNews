...
Key talk tracks as `主題|客群`. For schema version 1.1, produce all combinations of `全部`, `投資理財`, `貸款`, `新鮮事`, and `AI` with `高資產`, `定存`, `頂級卡`, and `貸款`. Never add fields ad hoc; update `data/schema.json` first when the contract changes.
